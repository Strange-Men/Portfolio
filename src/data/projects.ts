...
export const projects: PortfolioProject[] = [
  {
    ...
    highlights: [
      '搭建仓库过滤、AST 解析和结构化上下文流程，在 httpx / click / uvicorn 基准仓库实现平均 49.1% 文件降噪和 96.8% 上下文 token 压缩。',
      '设计 ReportContract、证据字段和 Mock / Real LLM 双模式，约束报告结构与引用依据，支持开发、测试和 CI 稳定回归。',
    ...
    proof:
      'README 记录 3 个基准仓库 benchmark、真实 LLM 单仓验证、pytest / ruff / audit_harness / Docker 结果；Demo 可输入 GitHub 仓库地址查看报告。',
  },
  {
    ...
    highlights: [
      '搭建上传、表格预览、DuckDB 查询和数据质量报告流程，让 CSV / Excel 数据可被自然语言问答和 SQL 工作台统一分析。',
      '实现 Mock / DeepSeek / Doubao / Mimo provider adapter 与 fallback，默认无 API Key 可运行，真实 Key 仅保存到后端环境变量。',
    ...
    proof:
      'README 记录 Docker Compose、本地前后端 200、Mock LLM 默认可用、Backend 559 passed、Frontend 1171 passed 和 master CI passed；Demo 可体验上传与 SQL 分析流程。',
  },
  {
    id: 'customerops',
    ...
    highlights: [
      '建设 JSONL 分层知识文档，覆盖 12 个客服场景，结合 BM25、查询扩展和元数据加权实现证据绑定检索。',
      '设计意图分类、规则路由和“结论 → 依据 → 操作建议”回答模板，并用 Profile 机制隔离前端与真实 LLM key。',
      '搭建 Evaluation Harness 与 131 条 Bad Case Bank，使 Answer Pass Rate 从 46.72% 提升至 60.66%，Recall@5 达到 90.00%，pytest 293 passed。',
    ],
    flow: [
      '用户查询进入意图识别与路由',
      'RAG 检索 / 工具查询 / 安全兜底',
    ...
    proof:
      'README 记录在线 Demo、Render API、三层评测体系、131 条 Bad Case、pytest 293 passed、ruff / 前端构建 / Docker Compose 与 Profile 验证。',
  },
];
