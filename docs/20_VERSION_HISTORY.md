...
| v1.34.0-og-image-polish | Phase 14-1 / 14-2 | 分享图优化阶段：已完成 Open Graph / Twitter 分享图方案研究，并在 `feat/v1.34.0-og-image-polish` 分支制作、接入 OG 分享图，当前处于分支验收中。 |
...
| v1.34.0-og-image-polish | Phase 14-2 分支验收中 | 设计并接入 Open Graph 分享图，用于社交平台和链接预览。Phase 14-1 研究阶段已完成，Phase 14-2 已在功能分支制作并接入 PNG 分享图。 |
